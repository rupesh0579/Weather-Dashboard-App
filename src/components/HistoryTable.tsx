...
// export default function HistoryTable() {
//   const { weatherHistory, clearHistory, getClasses } = usePrefStore();
//   const [sorting, setSorting] = useState<SortingState>([]);
//   const [globalFilter, setGlobalFilter] = useState("");

//   const { cardClass, labelClass, subTextClass, inputClass } = getClasses();
...
//   if (weatherHistory.length === 0) {
//     return (
//       <div className={`${cardClass} p-6 rounded-lg shadow text-center`}>
//         <h3 className={`text-xl font-bold mb-2 ${labelClass}`}>Weather History</h3>
//         <p className={subTextClass}>No weather data history available yet.</p>
...
//       <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 space-y-2 sm:space-y-0">
//         <h3 className={`text-xl font-bold ${labelClass}`}>Weather History</h3>
//         <div className="flex space-x-2">
//           <input
//             type="text"
//             value={globalFilter}
//             onChange={(e) => setGlobalFilter(e.target.value)}
//             placeholder="Search history..."
//             className={`border rounded-md px-3 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 ${inputClass}`}
//           />
//           <button
//             onClick={clearHistory}
//             className="bg-red-500 text-white px-3 py-1 rounded text-sm hover:bg-red-600 transition-colors"
...
//           <thead>
//             {table.getHeaderGroups().map((headerGroup) => (
//               <tr key={headerGroup.id} className={`border-b ${subTextClass}`}>
//                 {headerGroup.headers.map((header) => (
//                   <th
//                     key={header.id}
//                     className={`text-left px-4 py-3 font-medium ${labelClass} hover:bg-gray-50 cursor-pointer select-none`}
//                     onClick={header.column.getToggleSortingHandler()}
//                   >
//                     <div className="flex items-center space-x-1">
//                       <span>
...
//               >
//                 {row.getVisibleCells().map((cell) => (
//                   <td key={cell.id} className={`px-4 py-3 text-sm ${labelClass}`}>
//                     {flexRender(cell.column.columnDef.cell, cell.getContext())}
//                   </td>
//                 ))}
//               </tr>
...
//       <div className={`mt-4 text-sm ${subTextClass} text-center`}>
//         Showing {table.getRowModel().rows.length} of {weatherHistory.length} entries
//       </div>
//     </div>
//   );
